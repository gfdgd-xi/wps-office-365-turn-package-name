...
_convert_loong64() {
	...
	sed -e 's|^Package: wps-office$|Package: wps-office-365|g' \
	    -i "$DEBDIR"/metadata/control
	sed -e '/Recommends: ttf-mscorefonts-installer/d' \
	    -i "$DEBDIR"/metadata/control
	sed -e 's|^Replaces: kingsoft-office$|Replaces: kingsoft-office, wps-office|g' \
	    -i "$DEBDIR"/metadata/control
